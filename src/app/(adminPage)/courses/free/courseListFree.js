...
const CourseListFree = ({ courseData, idList }) => {
  ...
  const handleEditCourse = async (courseData) => {
    const firstChap = courseData?.chapters?.[0]
    const firstLession = firstChap?.lessions?.[0]
    router.push(`/courses/edit/${courseData?.slug}/${firstLession?.id}`)
  };
